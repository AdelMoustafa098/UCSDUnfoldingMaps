...
public class EarthquakeCityMap extends PApplet {
	...
	private void addKey() {	
		...
		float x_line = 42f;
		float y_line = 242f;
		line(x_line,y_line, x_line  + 15f, y_line + 15);
		line(x_line + 15f,y_line, x_line, y_line + 15);
		...
		float y = 100f;
		fill(110, 11, 4);
		triangle(x, y - 7.0f, x - 7.0f, y + 7.0f, x + 7.0f, y + 7.0f);
		
		
		
		fill(color(255, 247, 0));
	...
	private void printQuakes() 
	{
		...
		int ocean_quakeCounter =  0;
		java.util.HashMap<String,Integer> country_map = new java.util.HashMap<String,Integer>();
		
		for(Marker country : countryMarkers)
		{	
			String countryName = (String)country.getProperty("name");
			country_map.put(countryName, 0);
		}
		
		for(Marker earthquake : quakeMarkers)
		{	
			EarthquakeMarker em = (EarthquakeMarker)earthquake;
			if(em.isOnLand()) // earthquake is in land
			{
				String country_quake_name = (String)earthquake.getProperty("country");
				if(country_map.containsKey(country_quake_name))
				{
					country_map.put(country_quake_name, country_map.get(country_quake_name) + 1);
				}
		...
		for(String country : country_map.keySet())
		{	
			if(country_map.get(country) >= 1)
			{
				System.out.println(country + ": " + country_map.get(country));
			}	
		}
		
		System.out.println("OCEAN QUAKES: " + ocean_quakeCounter);
